...
WORKDIR /work/nassauer
COPY . .


RUN --mount=type=cache,target=/cargo  \ 
  --mount=type=cache,target=/work/nassauer/target \
  --mount=from=aya,target=/aya-rs \
...
ENV RUST_LOG="info"
COPY --from=builder /work/nassauer/out/nassauer /bin/nassauer
